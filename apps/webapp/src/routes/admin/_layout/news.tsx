...
import { createNewsFn, deleteNewsFn, listAllNewsFn, updateNewsFn } from "@webapp/server/functions/news";
...
function ExistingImage({ s3Key, isDeleted, onDelete, onRestore }: { s3Key: string; isDeleted: boolean; onDelete: () => void; onRestore: () => void }) {
	const { data: imageUrl } = useQuery({ queryKey: ["upload", "fileUrl", s3Key], queryFn: () => getFileUrlFn({ data: { s3Key } }), enabled: !!s3Key && !isDeleted });
