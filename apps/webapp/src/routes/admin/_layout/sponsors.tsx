...
import { MAX_UPLOAD_SIZE } from "@webapp/lib/image-config";
import { createSponsorFn, deleteSponsorFn, listSponsorsFn, updateSponsorFn } from "@webapp/server/functions/sponsors";
...
function CurrentLogoDisplay({
	...
	const { data: logoUrl } = useQuery({ queryKey: ["upload", "fileUrl", logoS3Key], queryFn: () => getFileUrlFn({ data: { s3Key: logoS3Key || "" } }), enabled: !!logoS3Key && !deleteLogo });
...
function SponsorCard({ sponsor, onEdit }: { sponsor: SponsorInput & { id: string }; onEdit: (sponsor: SponsorInput & { id: string }) => void; onDelete: (id: string) => void; isDeleting: boolean }) {
	const { data: logoUrl } = useQuery({ queryKey: ["upload", "fileUrl", sponsor.logoS3Key], queryFn: () => getFileUrlFn({ data: { s3Key: sponsor.logoS3Key || "" } }), enabled: !!sponsor.logoS3Key });
