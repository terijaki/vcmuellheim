...
function entityFor(entities: AuthDb, model: string): MinimalEntity {
	switch (model) {
		...
		case "session":
			return entities.session as unknown as MinimalEntity;
		case "account":
			if (entities.account) return entities.account;
			throw new Error('"account" model requested but not provided in AuthDb. Add an account entity for email/password auth.');
		default:
			throw new Error(`Unknown better-auth model: "${model}". Expected user | account | verification | session`);
	}
}

/** Join user data onto a session item when the caller requests it. */
async function withSessionUserJoin(getDb: () => AuthDb, item: ElectroItem | null, model: string, join: unknown): Promise<ElectroItem | null> {
	if (!item) return null;
...
/**
 * Create the DynamoDB adapter backed by the provided ElectroDB entity getter.
 * Pass the lazy `db` function for production; pass a fake in tests.
 */
export function createDynamoDBAdapter(getDb: () => AuthDb) {
		...
		adapter: ({ getModelName }) => {
			return {
				async create({ model, data }) {
					...
					const entity = entityFor(getDb(), resolvedModel);
				...
				async findOne({ model, where, select, join }) {
					...
					return withSessionUserJoin(getDb, selectFields(matched, select), resolvedModel, join) as ReturnType<typeof Object.assign>;
				},
				...
				async update({ model, where, update }) {
					...
					await entityFor(getDb(), resolvedModel).put(updatedItem).go();
				...
				async delete({ model, where }) {
					...
					const idWhere = where.find((w) => w.field === "id");
					if (idWhere) {
						await entityFor(getDb(), resolvedModel)
...
/** Production singleton backed by the lazy env-configured db(). */
export const dynamoDBAdapter = createDynamoDBAdapter(db);
