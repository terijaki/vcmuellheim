...
function entityFor(model: string) {
const entities = db();
switch (model) {
case "user":
...
/**
 * Invoke entity.put with an untyped item.
 * The type cast is necessary because entityFor() returns a union of ElectroDB entity types
 * and TypeScript cannot narrow the put() signature through the union without explicit assistance.
 */
function putEntityItem(entity: ReturnType<typeof entityFor>, item: ElectroItem) {
	return (entity as { put: (data: ElectroItem) => { go: () => Promise<unknown> } }).put(item);
}
...
await putEntityItem(entity, item).go();
...
if (emailWhere) {
const { data: items } = await db().user.query.byEmail({ email: emailWhere.value as string }).go({ limit: 1 });
...
if (tokenWhere) {
const { data: items } = await db().session.query.byToken({ token: tokenWhere.value as string }).go({ limit: 1 });
const first = (items[0] as ElectroItem | undefined) ?? null;
return withSessionUserJoin(selectFields(first ?? {}, select), resolvedModel, join) as ReturnType<typeof Object.assign>;
}
}
...
if (identifierWhere) {
const { data: items } = await db().verification.query.byIdentifier({ identifier: identifierWhere.value as string }).go({ pages: "all" });
...
if (userIdWhere) {
const { data: items } = await db().session.query.byUserId({ userId: userIdWhere.value as string }).go({ pages: "all" });
const filtered = (items as ElectroItem[]).filter((item) => matchesWhere(item, where));
const sorted = sortItems(filtered, sortBy);
return sorted.slice(startIndex, startIndex + limit) as T[];
}
...
await putEntityItem(entityFor(resolvedModel), updatedItem).go();
...
const idWhere = where.find((w) => w.field === "id");
if (idWhere) {
await entityFor(resolvedModel).delete({ id: idWhere.value as string }).go();
return;
}
...
if (session?.id) {
await db().session.delete({ id: session.id as string }).go();
}
return;
}
}
...
if (existing?.id) {
await entityFor(resolvedModel).delete({ id: existing.id as string }).go();
}
},
...
for (const item of items) {
if (item.id) {
await entityFor(resolvedModel).delete({ id: item.id as string }).go();
}
}
return items.length;
},
