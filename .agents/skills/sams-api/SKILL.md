...
```
vp exec tsx --env-file=.env.local .agents/skills/sams-api/sams.ts <resource> [uuid] [subresource] [--query key=value ...]
```
...
```bash
vp exec tsx --env-file=.env.local .agents/skills/sams-api/sams.ts seasons --query size=1
```
