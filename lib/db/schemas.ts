...
export const newsSchema = z.object({
  ...
  sharedToMastodon: z
    .boolean()
    .optional()
    .describe("Whether this news article has been shared to Mastodon"),
});
  ...
  .object({
    ...
    location: z
      .object({
        ...
        address: z
          .union([z.string(), samsProjectionMatchLocationAddressSchema])
          .nullish()
          .transform((address) => (typeof address === "string" ? undefined : address)),
      })
      .nullish(),
    _embedded: z
      .object({
...
export const volunteerRoleSchema = z
  ...
  .refine((r) => r.maxCapacity === undefined || r.maxCapacity >= r.minCapacity, {
    message: "maxCapacity muss größer oder gleich minCapacity sein",
  });
...
export const volunteerShiftSchema = z.object({
  ...
  archivedAt: z.iso
    .datetime()
    .optional()
    .describe("Set when the shift is archived; hides it from public signup form"),
...
export const volunteerEventSchema = z.object({
  ...baseEntityFields,
  type: z
    .literal("volunteerEvent")
    .default("volunteerEvent")
    .describe("Entity type, primary key for GSI queries"),
  title: z.string().min(1).max(200),
  description: z.string().optional(),
  location: z.string().optional(),
  locationUrl: z
    .string()
    .optional()
    .describe("Optional URL for the location (e.g. Google Maps link)"),
  organizerName: z
    .string()
    .min(1)
    .max(200)
    .describe("Name of the event organizer — shown in email contact line"),
  organizerEmail: z
    .email()
    .trim()
    .max(200)
  ...
  archivedAt: z.iso
    .datetime()
    .optional()
    .describe("Set when the event is archived; hides it from public view"),
});
...
export const volunteerSignupDataSchema = z.object({
  ...
  mobilePhone: z
    ...
    .max(30)
    .optional()
    .describe("Optional mobile phone number of the volunteer"),
  emergencyContact: z
    .string()
    .trim()
    .max(30)
    .optional()
    .describe("Emergency contact phone number — required for minors (under 18 at shift start)"),
  note: z
    .string()
    .trim()
    .max(1000)
...
export const volunteerSignupSchema = volunteerSignupDataSchema.extend({
  ...
  assignedRoleId: z
    .uuid()
    .optional()
    .describe("Role assigned by admin, overrides helper preference"),
});
