...
/**
 * Generic repository for SAMS DynamoDB CRUD operations
 */
export class SamsRepository<T extends Partial<BaseEntity>> {
	constructor(private config: RepositoryConfig & { identifier: string }) {}
	...
	async update(id: string, updates: Partial<Omit<T, "id" | "createdAt">>): Promise<T> {
		...
		// Build update expression from provided fields (skip undefined values)
		Object.entries(updates).forEach(([key, value]) => {
			if (key !== this.config.identifier && key !== "createdAt" && value !== undefined) {
				const placeholder = `:${key}`;
				const attributeName = `#${key}`;
				updateExpressions.push(`${attributeName} = ${placeholder}`);
				expressionAttributeValues[placeholder] = value;
		...
		const result = await docClient.send(
			new UpdateCommand({
				...
				ExpressionAttributeNames: expressionAttributeNames,
				ReturnValues: "ALL_NEW",
				ConditionExpression: `attribute_exists(${this.config.identifier})`, // Ensure item exists
			}),
		);
