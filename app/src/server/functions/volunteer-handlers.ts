...
export async function getPublicVolunteerEvent(data: {
  id: string;
}): Promise<PublicVolunteerEventData> {
  ...
  const signupsResult = await db()
    ...
    .go({ pages: "all" });
  const allSignups = parseServerArray(
    volunteerSignupSchema,
    signupsResult.data,
    "Failed to parse signups",
  );
  ...
  for (const signup of allSignups) {
    if (signup.status === "confirmed") {
      ...
      if (signup.assignedRoleId) {
        ...
        signupCounts[signup.shiftId][signup.assignedRoleId] =
          (signupCounts[signup.shiftId][signup.assignedRoleId] ?? 0) + 1;
      }
  ...
  if (shift && shift.roles.length > 0) {
    ...
    const availableRole = signupData.preferredRoleIds
      ...
      .find(
        (role) =>
          role !== undefined &&
          (role.maxCapacity === undefined || (roleCountMap[role.id] ?? 0) < role.maxCapacity) &&
          (role.minAge === undefined || userAgeAtShift >= role.minAge),
      );
    if (availableRole) {
      await db()
        .volunteerSignup.patch({ id: signup.id })
        .set({ assignedRoleId: availableRole.id, updatedAt: new Date().toISOString() })
        .go();
...
export async function sendBulkVolunteerEventEmail(data: {
  eventId: string;
  subject: string;
  htmlBody: string;
