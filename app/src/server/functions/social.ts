 ...
 * a few times per day. This route never calls the Behold API directly, eliminating
 * any risk of hitting the 1200 views/month free-tier limit on the main request path.
 */

import { createServerFn } from "@tanstack/react-start";
import type { BeholdPost } from "@/lambda/social/types";
import { createCacheKey } from "@utils/cache";
import { readSamsCacheEntry } from "../sams-ddb-cache";
...
export const getInstagramPostsFn = createServerFn({ method: "GET" }).handler(async (): Promise<BeholdPost[]> => {
	try {
		const cached = await readSamsCacheEntry<BeholdPost[]>(BEHOLD_CACHE_KEY, Infinity);
		return cached ?? [];
	} catch {
		return [];
	}
