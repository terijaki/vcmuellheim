...
function RolesManager({
  ...
  roles: RoleFormValue[];
  onRolesChange: (roles: RoleFormValue[]) => void;
}) {
  ...
  return (
    <>
      <Modal opened={deleteModalOpened} onClose={closeDeleteModal} title="Rolle löschen?" size="sm">
        <Text size="sm">
          Soll diese Rolle wirklich gelöscht werden? Bestehende Anmeldungen verlieren ihre
          Rollenzuweisung, bleiben aber erhalten und können neu zugewiesen werden.
        </Text>
            ...
            <Fieldset key={role.id} legend={role.label} bg="gray.0">
              <Group gap="xs" align="flex-end">
                ...
                <NumberInput
                  ...
                  label="Ziel"
                  min={1}
                  value={role.minCapacity}
                  onChange={(val) => updateRole(index, { minCapacity: Number(val) || 1 })}
                  w={70}
                />
                <NumberInput
                  size="xs"
                  ...
                  onChange={(val) =>
                    updateRole(index, {
                      maxCapacity: val === "" || val === 0 ? null : Number(val) || 1,
                    })
                  }
                  w={70}
                />
...
export function ShiftsManager({
  ...
  const [shiftActionModal, setShiftActionModal] = useState<{
    index: number;
  ...
  const addShift = () => {
    ...
    onShiftsChange([
      ...shifts,
      { id: newId, label: "", startDate: null, endDate: null, roles: [] },
    ]);
  ...
  return (
    <Box>
      <Modal
        opened={!!shiftActionModal}
        onClose={() => setShiftActionModal(null)}
        title={shiftActionModal?.action === "delete" ? "Schicht löschen?" : "Schicht archivieren?"}
        size="sm"
      >
        <Text size="sm">
        ...
        <Group justify="flex-end" mt="md">
          ...
          <Button
            color={shiftActionModal?.action === "delete" ? "red" : "orange"}
      ...
      <Accordion
        multiple
        variant="separated"
        value={expandedShifts}
        onChange={setExpandedShifts}
              ...
              <Accordion.Panel>
                <Stack gap="sm">
                  <Group align="flex-end" gap="xs">
                    ...
                    {signupCount > 0 ? (
                      <Tooltip
                        label={`${signupCount} Anmeldung${signupCount !== 1 ? "en" : ""} – nur Archivieren möglich`}
                      >
                        <ActionIcon
                          color="orange"
                          variant="subtle"
                          onClick={() => setShiftActionModal({ index, action: "archive" })}
                          mb={8}
                        >
                          <Archive size={16} />
                        </ActionIcon>
                      ...
                      <ActionIcon
                        color="red"
                        variant="subtle"
                        onClick={() => setShiftActionModal({ index, action: "delete" })}
                        mb={8}
                      >
                        <Trash2 size={16} />
                      </ActionIcon>
                  ...
                  <SimpleGrid cols={{ base: 1, xs: 2 }}>
                    <DateTimePicker
                      ...
                      onChange={(val) =>
                        updateShift(index, { startDate: val ? new Date(val) : null })
                      }
                      locale="de"
                    ...
                    <DateTimePicker
                      ...
                      value={shift.endDate}
                      onChange={(val) =>
                        updateShift(index, { endDate: val ? new Date(val) : null })
                      }
                      locale="de"
                  ...
                  <RolesManager
                    roles={shift.roles}
                    onRolesChange={(roles) => updateShift(index, { roles })}
                  />
                </Stack>
              </Accordion.Panel>
            </Accordion.Item>
