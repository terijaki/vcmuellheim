...
export default function HomeKontakt() {
  ...
  const treasurer = members?.items.find(
    (member) =>
      member.roleTitle?.includes("Kassier") || member.roleTitle?.includes("Schatzmeister:in"),
  );
  const membershipManager = members?.items.find((member) =>
    member.roleTitle?.includes("Mitgliederverwaltung"),
  );
            ...
            <SimpleGrid cols={{ base: 1, xs: 2, sm: 3 }} spacing="lg" verticalSpacing="lg">
              <ContactItem title="Jugendschutz">
                <Text component="div" size="sm">
                  <ContactRouteLink to="/jugendschutz" label="Hier" /> findest du alle Informationen
                  zum Jugendschutz.
                </Text>
              </ContactItem>
              ...
              <ContactItem title="Vereinsbeitritt">
                <Text component="div" size="sm">
                  Hier gehts zur <ContactRouteLink to="/beitragsordnung" label="Beitragsordnung" />{" "}
                  und hier zum{" "}
                  <ContactLink
          ...
          <Stack gap="md">
            ...
            <SimpleGrid cols={{ base: 1, xs: 2, sm: 3 }} spacing="lg" verticalSpacing="lg">
              <ContactItem title="Mitgliedschaft">
                <Text component="div" size="sm">
                  Deine Mitgliedschaft verwaltest du selbst in{" "}
                  <ContactLink
                    href="https://vcm.kurabu.com/de/login"
                    target="_blank"
                    label="KURABU"
                    icon={<FaArrowUpRightFromSquare />}
                  />
                  . Bei Fragen melde dich bei{" "}
                  {membershipManager?.name ? `${membershipManager.name} ` : ""}
                  <ContactLink
              ...
              <ContactItem title="Beitragszahlung">
                <Text component="div" size="sm">
                  Bei Fragen zu deinen Beiträgen melde dich bitte bei{" "}
              ...
              <ContactItem title="Spesenabrechnung">
                <Text component="div" size="sm">
                  Hier findest du die{" "}
                  <ContactLink
                    href="/docs/spesenabrechnung.xlsx"
                    label="Spesenabrechnung"
                    icon={<FaFileExcel />}
                  />{" "}
                  und{" "}
                  <ContactLink
                    href="/docs/trainerverguetung.xlsx"
              ...
              <ContactItem title="Vereinsbus">
                <Text component="div" size="sm">
                  Hier findest du die{" "}
                  <ContactLink href="/bus" label="Busverfügbarkeit" icon={<FaBus />} />
                </Text>
              </ContactItem>
            </SimpleGrid>
          </Stack>
...
function ContactRouteLink({
  ...
  to: "/jugendschutz" | "/beitragsordnung" | "/brand";
  label: string;
}) {
  return (
