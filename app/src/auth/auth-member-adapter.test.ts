...
async function findOne(args: { model: string; where: Array<{ field: string; value: unknown; operator?: string }>; select?: string[] }) {
...
describe("findOne — user model", () => {
	...
	it("finds an Admin member by privateEmail and maps it to email in auth view", async () => {
		const result = (await findOne({ model: "user", where: [{ field: "email", value: "admin@example.com" }] })) as Record<string, unknown>;
	...
	it("returns null when no member has that email", async () => {
		const result = await findOne({ model: "user", where: [{ field: "email", value: "unknown@example.com" }] });

		expect(result).toBeNull();
	});

	it("returns null for a member without Admin or Moderator role even if email matches", async () => {
		const result = await findOne({ model: "user", where: [{ field: "email", value: "norole@example.com" }] });

		expect(result).toBeNull();
	});

	it("finds a member by id and maps privateEmail to email", async () => {
		const result = (await findOne({ model: "user", where: [{ field: "id", value: "m-mod" }] })) as Record<string, unknown>;
	...
	it("returns null when looking up by id for a non-existent member", async () => {
		const result = await findOne({ model: "user", where: [{ field: "id", value: "does-not-exist" }] });

		expect(result).toBeNull();
	});

	it("applies field selection when select is provided", async () => {
		const result = (await findOne({ model: "user", where: [{ field: "email", value: "admin@example.com" }], select: ["id", "email"] })) as Record<string, unknown>;
...
describe("findOne — non-user models return null", () => {
	...
	it("returns null for verification model", async () => {
		const result = await findOne({ model: "verification", where: [{ field: "identifier", value: "admin@example.com" }] });
		expect(result).toBeNull();
	});

	it("returns null for account model", async () => {
		const result = await findOne({ model: "account", where: [{ field: "userId", value: "m-admin" }] });
		expect(result).toBeNull();
	});
});
