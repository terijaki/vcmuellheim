	...
	adapter: ({ getModelName }) => ({
		async findOne({ model, where, select }) {
			...
			if (emailWhere) {
				...
				const { data: byPrivate } = await db().member.query.byPrivateEmail({ privateEmail: inputEmail }).go({ limit: 1 });
				const privateMatch = (byPrivate as ElectroItem[]).find((m) => m.authRole === "Admin" || m.authRole === "Moderator");
				if (privateMatch) {
				...
				const { data: byProxy } = await db().member.query.byProxyEmail({ proxyEmail: inputEmail }).go({ limit: 1 });
				const proxyMatch = (byProxy as ElectroItem[]).find((m) => m.authRole === "Admin" || m.authRole === "Moderator");
				if (proxyMatch) {
