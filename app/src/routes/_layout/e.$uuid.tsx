      ...
      <Stack gap="xl" pb="xl">
        ...
        {verifyMutation.isSuccess && verifyMutation.data?.success && (
          <Alert color="green" icon={<CheckCircle size={18} />} title="Anmeldung bestätigt!">
            Deine Anmeldung wurde erfolgreich bestätigt. Du erhältst in Kürze eine Bestätigungsmail
            mit dem Termin.
          </Alert>
        )}
        {verifyMutation.isSuccess && !verifyMutation.data?.success && (
        ...
        {event.shifts.length > 0 &&
          (() => {
            ...
            const earliestShift = event.shifts.reduce((a, b) =>
              dayjs(a.startDate).isBefore(dayjs(b.startDate)) ? a : b,
            );
            const latestShift = event.shifts.reduce((a, b) =>
              dayjs(a.endDate ?? a.startDate).isAfter(dayjs(b.endDate ?? b.startDate)) ? a : b,
            );
      ...
      <Stack gap="md">
            ...
            return (
              <Card key={role.id} withBorder p="xs" bg="gray.0">
                <Group justify="space-between">
                  ...
                  <Badge
                    size="md"
                    color={count === 0 ? "red" : role.minCapacity > count ? "yellow" : "green"}
                    variant="light"
                  >
        ...
        {submitted && (
          <Alert ref={alertRef} color="green" title="Anmeldung eingegangen!">
            Bitte überprüfe dein E-Mail-Postfach und bestätige deine Anmeldung innerhalb von 72
            Stunden.
          </Alert>
        )}
      </Stack>
...
function SignupForm({ event, shiftLabel, shiftId, roles, onSuccess, onCancel }: SignupFormProps) {
  ...
  const makeRenderRoleOption =
    ...
    ({ option }: { option: { value: string; label: string } }) => {
      ...
      const isIneligible =
        ageAtShift !== null && role?.minAge !== undefined && ageAtShift < role.minAge;
      return (
  ...
  const mutation = useMutation({
    mutationFn: (formData: Parameters<typeof createVolunteerSignupFn>[0]["data"]) =>
      createVolunteerSignupFn({ data: formData }),
    onSuccess,
  });
  ...
  return (
    ...
    >
      <Stack gap="sm">
        ...
        <Text size="sm">
          Vielen Dank, dass du dich anmelden möchtest! Damit wir die Organisation erleichtern und im
          Nachgang die Kommunikation mit dir sicherstellen können, fülle bitte folgende
          Informationen aus.
        </Text>
        ...
        {roles.length > 1 && (
          <form.Subscribe selector={(state) => state.values.dateOfBirth}>
            {(dateOfBirth) => {
              const ageAtShift = dateOfBirth
                ? dayjs(shiftStartDate).diff(dayjs(dateOfBirth), "year")
                : null;
