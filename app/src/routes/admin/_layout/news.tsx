...
function NewsPage() {
  ...
  const createMutation = useMutation({
    ...
    onError: (error) => {
      setUploading(false);
      notification.error({
        message: (error as Error).message || "News-Artikel konnte nicht erstellt werden",
      });
    },
  });
  const updateMutation = useMutation({
    ...
    onError: (error) => {
      setUploading(false);
      notification.error({
        message: (error as Error).message || "News-Artikel konnte nicht aktualisiert werden",
      });
    },
  });
  const deleteMutation = useMutation({
    ...
    onError: (error) => {
      notification.error({
        message: (error as Error).message || "News-Artikel konnte nicht gelöscht werden",
      });
    },
  });
  ...
  const handleSubmit = async (
    ...
    formData: typeof defaultFormValues,
  ) => {
  ...
  return (
    <Stack gap="md">
      ...
      <Modal
        opened={opened}
        onClose={close}
        title={editingId ? "News bearbeiten" : "News erstellen"}
        size="100%"
        fullScreen={isMobile}
      >
                  ...
                  <Group gap="sm" mb="md">
                    {formData.imageS3Keys.map((s3Key) => (
                      <ExistingImage
                        ...
                        onRestore={() =>
                          setImagesToDelete(imagesToDelete.filter((k) => k !== s3Key))
                        }
                      />
                    ))}
                  </Group>
        ...
        <>
          ...
          {searchQuery || statusFilter !== "all" ? (
            <Text size="sm" c="dimmed" mt="md">
              {filteredNews.length} von {news?.items.length || 0} Artikel
              {filteredNews.length !== 1 ? "n" : ""}
            </Text>
          ) : null}
        </>
      ) : (
...
function ExistingImage({
  ...
  return (
    <Card withBorder p="xs" pos="relative" w={120}>
      ...
      {imageUrl ? (
        <Image src={imageUrl} height={100} fit="cover" alt="Bild" radius="sm" />
      ) : (
        <Box
          h={100}
          bg="gray.1"
