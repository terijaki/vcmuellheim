...
import { buildTemplateData, type EventFormInitialData } from "@webapp/components/admin/volunteer-event/types";
import dayjs from "dayjs";
import "dayjs/locale/de";
import { Archive, ChevronDown, ChevronUp, Copy, EllipsisVertical, ExternalLink, Link, Mail, Plus, SquarePen, Trash2 } from "lucide-react";
									...
									<Menu shadow="md" position="bottom-end">
										...
										<Menu.Dropdown>
											...
											<Menu.Item
												...
												onClick={() => navigator.clipboard.writeText(deeplink).catch(() => notification.error({ message: "Link konnte nicht kopiert werden" }))}
											>
												Link kopieren
											</Menu.Item>
										...
										<Group justify="space-between" align="flex-start">
											...
											<Menu shadow="md" position="bottom-end">
												...
												<Menu.Dropdown>
													...
													<Menu.Item
														leftSection={<Link size={14} />}
														onClick={() => navigator.clipboard.writeText(`${appBaseUrl}/e/${event.id}`).catch(() => notification.error({ message: "Link konnte nicht kopiert werden" }))}
													>
														Link kopieren
													</Menu.Item>
													<Menu.Item leftSection={<Mail size={14} />} onClick={() => navigate({ to: "/admin/volunteer-event/$eventId/message", params: { eventId: event.id } })}>
