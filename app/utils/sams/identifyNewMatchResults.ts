...
import { getClubsTeamIds } from "./clubs";
...
export async function identifyNewMatchResults(matchData: string) {
	...
	const teamIds = await getClubsTeamIds("id");
	const filteredMatches = matches.filter((match) => match.results && new Date(match.dateObject) >= todayMinusRange && (teamIds && (teamIds.includes(match.team[0].id) || teamIds.includes(match.team[1].id))));
