...
export default async function getPlayers(teamId: (number | string)[]) {
	...
	for (const id of teamId) {
		...
		let playerDataAge = new Date("1980-01-01"); // fake date in case the file does not exist
		...
		if (fs.existsSync(targetFile)) {
			...
			playerDataAge = existingJsonObject.date;
		}
		...
		if (cacheTTLDate > new Date(playerDataAge).getTime()) {
