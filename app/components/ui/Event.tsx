import type getEvents from "@/app/utils/getEvents";
import Link from "next/link";
import { FaLocationDot as IconLocation } from "react-icons/fa6";

export default function Event(props: Awaited<ReturnType<typeof getEvents>>[number] & { totalSiblings?: number }) {
	...
	if (props.title && props.start) {
		return (
			...
			>
				<time className="block text-sm pt-1" data-label="datetime">
					{props.end
					...
					{props.start &&
						...
						` ${props.start.toLocaleString("de-DE", { timeStyle: "short" })} Uhr`}
					{props.end && <span> - {props.end.toLocaleString("de-DE", { dateStyle: "medium" })}</span>}
				</time>
				<div className="py-1 font-bold" data-label="title">
					{props.url ? (
						<Link key={props.title + props.start} href={props.url} className={"gap-x-4 text-onyx hover:text-blumine"}>
							{props.title}
						</Link>
					) : (
						<>{props.title}</>
				...
				<div className="pb-1 sm:py-1 mr-3 sm:mr-0" data-label="location">
					{props.location?.city && props.location.postalCode && props.location.street && props.location.name ? (
						<Link
						...
						>
							...
							<span className="text-sm md:text-base ml-1">
								{" "}
								({props.location.street.replace("straße", "str.").replace("strasse", "str.")})
							</span>
						</Link>
					) : (
						""
