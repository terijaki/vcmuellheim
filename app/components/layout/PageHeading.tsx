...
export default function PageHeading(props: { title: string; subtitle?: string; subtitleDate?: boolean }) {
	...
	return (
		...
		>
			...
			<div className="grid grid-cols-main-grid py-6">
				...
				<h1 className="text-white text-2xl lg:text-3xl font-montserrat font-semibold col-center-content text-balance text-center lg:text-left">{props.title}</h1>
				{props.subtitle && props.subtitleDate && (
					<time
						className="col-center-content text-white text-sm font-thin text-balance text-center lg:text-left mt-1"
						dateTime={props.subtitle}
					>
						{new Date(props.subtitle).toLocaleString("de-DE", { day: "numeric", month: "short", year: "numeric" })}
					</time>
				)}
				{props.subtitle && !props.subtitleDate && <p className="col-center-content text-white text-sm font-thin text-balance text-center lg:text-left mt-1">{props.subtitle}</p>}
			</div>
		</section>
	);
}
