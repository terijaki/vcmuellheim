...
export default async function MemberCard({ member, show, dark }: { member: Member; show?: "roles" | "email" | "phone"; dark?: boolean }) {
	...
	return (
		<Card component={Link} data-member-id={id} href={email ? `mailto:${email}` : ""} scroll={false} p={0} withBorder bg={dark ? "onyx" : undefined}>
			<Stack gap={0}>
				<Group gap={0}>
					<Stack bg={dark ? "blumine" : "lion"} w={58} h={64} c="white" pos="relative" align="center" justify="center">
						{avatarUrl ? <Image fill src={avatarUrl} alt={name} style={{ objectFit: "cover" }} /> : <IconAvatar width={"100%"} height={"100%"} />}
					</Stack>
